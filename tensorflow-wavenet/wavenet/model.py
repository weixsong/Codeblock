...
def get_variable_on_cpu(name, shape):
    '''Create a convolution filter variable with the specified name and shape,
    and initialize it using Xavier initialition.
    The variable is created on CPU'''
...
def get_bias_variable_on_cpu(name, shape):
    '''Create a bias variable with the specified name and shape and initialize
    it to zero.
    The variable is created on CPU'''
    with tf.device("/cpu:0"):
...
def create_embedding_table(name, shape):
    if shape[0] == shape[1]:
        ...
        return tf.Variable(initial_val, name=name, trainable=False)
    else:
        return create_variable(name, shape)


def get_embedding_table_on_cpu(name, shape):
    with tf.device("/cpu:0"):
        if shape[0] == shape[1]:
            # Make a one-hot encoding as the initial value.
            initial_val = np.identity(n=shape[0], dtype=np.float32)
            return tf.get_variable(name, initializer=initial_val, trainable=False)
        else:
            return get_variable_on_cpu(name, shape)
...
class WaveNetModel(object):
    ...
    def _create_variables(self):
        ...
        # switch to get_variable to reuse variable between GPUs
        create_variable = get_variable_on_cpu
        create_bias_variable = get_bias_variable_on_cpu
        create_embedding_table = get_embedding_table_on_cpu
