...
def create_train_data():
    ...
    for row in reader:
        ...
        if hp.min_len <= len(text) <= hp.max_len:
            texts.append(np.array([char2idx[char]
...
def load_train_data():
    ...
    # We use a single mini-batch for training to overfit it.
    if hp.sanity_check:
        texts, sound_files = texts[:hp.batch_size] * \
            1000, sound_files[:hp.batch_size] * 1000
    else:
        texts, sound_files = texts[
            :-hp.num_samples], sound_files[:-hp.num_samples]
    return texts, sound_files
...
def load_eval_data():
    ...
    # We generate samples for the same texts as the ones we've used for
    # training.
    if hp.sanity_check:
