...
def normalize(inputs,
    ...
    if type == "bn":
        ...
        # pay attention to the fact that fused_batch_norm requires shape to be
        # rank 4 of NHWC.
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            mean, variance = tf.nn.moments(
                inputs, [reduction_axis], keep_dims=True)
...
def prenet(inputs, is_training=True, scope="prenet", reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        outputs = tf.layers.dense(
            inputs, units=hp.embed_size, activation=tf.nn.relu, name="dense1")
        outputs = tf.nn.dropout(
            outputs, keep_prob=.5 if is_training == True else 1., name="dropout1")
        outputs = tf.layers.dense(
            outputs, units=hp.embed_size // 2, activation=tf.nn.relu, name="dense2")
        outputs = tf.nn.dropout(
            outputs, keep_prob=.5 if is_training == True else 1., name="dropout2")
