...
def encode(inputs, is_training=True, scope="encoder", reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # Encoder CBHG
        # Conv1D bank
        enc = conv1d_banks(prenet_out, K=hp.encoder_num_banks,
                           is_training=is_training)  # (N, T, K * E / 2)

        # Max pooling
        enc = tf.layers.max_pooling1d(
            enc, 2, 1, padding="same")  # (N, T, K * E / 2)

        # Conv1D projections
        enc = conv1d(enc, hp.embed_size // 2, 3,
...
def decode1(decoder_inputs, memory, is_training=True, scope="decoder1", reuse=None):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        # Attention RNN
        dec = attention_decoder(
            dec, memory, num_units=hp.embed_size)  # (N, T', E)

        # Decoder RNNs
        dec += gru(dec, hp.embed_size, False,
